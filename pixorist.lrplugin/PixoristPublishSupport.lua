...
PixoristPublishServiceProvider.lua
Publish-specific portions of Lightroom Pixorist uploader
...
--- The <i>service definition script</i> for a publish service provider associates
 ...
 -- <p>The <code>PixoristPublishSupport.lua</code> file of the Pixorist sample plug-in provides
 ...
 --     <code>PixoristExportServiceProvider.lua</code> file.</p>
...
publishServiceProvider.small_icon = 'pixorist-logo.png'
...
publishServiceProvider.titleForPublishedCollection = LOC "$$$/Pixorist/TitleForPublishedCollection=Photoset"
...
publishServiceProvider.titleForPublishedCollection_standalone = LOC "$$$/Pixorist/TitleForPublishedCollection/Standalone=Photoset"
...
-- publishServiceProvider.titleForPublishedCollectionSet = "(something)" -- not used for Pixorist plug-in
...
--publishServiceProvider.titleForPublishedCollectionSet_standalone = "(something)" -- not used for Pixorist plug-in
...
publishServiceProvider.titleForPublishedSmartCollection = LOC "$$$/Pixorist/TitleForPublishedSmartCollection=Smart Photoset"
...
publishServiceProvider.titleForPublishedSmartCollection_standalone = LOC "$$$/Pixorist/TitleForPublishedSmartCollection/Standalone=Smart Photoset"
...
function publishServiceProvider.getCollectionBehaviorInfo( publishSettings )
    ...
    return {
        defaultCollectionName = LOC "$$$/Pixorist/DefaultCollectionName/Photostream=Photostream",
        defaultCollectionCanBeDeleted = false,
        canAddCollection = true,
        maxCollectionSetDepth = 0,
            -- Collection sets are not supported through the Pixorist sample plug-in.
    }
...
publishServiceProvider.titleForGoToPublishedCollection = LOC "$$$/Pixorist/TitleForGoToPublishedCollection=Show in Pixorist"
...
publishServiceProvider.titleForGoToPublishedPhoto = LOC "$$$/Pixorist/TitleForGoToPublishedCollection=Show in Pixorist"
...
function publishServiceProvider.deletePhotosFromPublishedCollection( publishSettings, arrayOfPhotoIds, deletedCallback )
    ...
    for i, photoId in ipairs( arrayOfPhotoIds ) do
        PixoristAPI.deletePhoto( photoId )
...
function publishServiceProvider.metadataThatTriggersRepublish( publishSettings )
    ...
    return {
        ...
        -- also (not used by Pixorist sample plug-in):
            -- customMetadata = true,
...
--[[ Not used for Pixorist plug-in. This is an example of how this function might work.
...
--- (optional) This plug-in defined callback function is called when new or updated
 ...
 -- the user-specified sort order should be followed as-is or reversed. The Pixorist
 -- sample plug-in uses this to reverse the order on the Photostream so that photos
 -- appear in the Pixorist web interface in the same sequence as they are shown in the
 -- library grid.
 -- <p>This is not a blocking call. It is called from within a task created
 -- using the <a href="LrTasks.html"><code>LrTasks</code></a> namespace. In most
 -- cases, you should not need to start your own task within this function.</p>
...
function publishServiceProvider.imposeSortOrderOnPublishedCollection( publishSettings, info, remoteIdSequence )
    ...
    if photosetId then
        ...
        local existingPhotoSequence = PixoristAPI.listPhotosFromPhotoset( publishSettings, { photosetId = photosetId } )
        ...
        if existingPhotoSequence and existingPhotoSequence.primary then
            PixoristAPI.setPhotosetSequence( publishSettings, {
...
-- publishServiceProvider.disableRenamePublishedCollection = true -- not used for Pixorist sample plug-in
...
-- publishServiceProvider.disableRenamePublishedCollectionSet = true -- not used for Pixorist sample plug-in
...
function publishServiceProvider.renamePublishedCollection( publishSettings, info )
    ...
    if info.remoteId then
        ...
        PixoristAPI.createOrUpdatePhotoset( publishSettings, {
...
function publishServiceProvider.deletePublishedCollection( publishSettings, info )
    ...
    import 'LrFunctionContext'.callWithContext( 'publishServiceProvider.deletePublishedCollection', function( context )
        ...
        local progressScope = LrDialogs.showModalProgressDialog {
                            title = LOC( "$$$/Pixorist/DeletingCollectionAndContents=Deleting photoset ^[^1^]", info.name ),
        ...
        if info and info.photoIds then
            ...
            for i, photoId in ipairs( info.photoIds ) do
                ...
                PixoristAPI.deletePhoto( publishSettings, { photoId = photoId } )
        ...
        if info and info.remoteId then
            ...
            PixoristAPI.deletePhotoset( publishSettings, {
                                photosetId = info.remoteId,
                                suppressError = true,
                                    -- Pixorist has probably already deleted the photoset
...
--
--      local comments = PixoristAPI.getComments( publishSettings, {
...
publishServiceProvider.titleForPhotoRating = LOC "$$$/Pixorist/TitleForPhotoRating=Favorite Count"
...
--  for i, photoInfo in ipairs( arrayOfPhotoInfo ) do
--
--      local rating = PixoristAPI.getNumOfFavorites( publishSettings, { photoId = photoInfo.remoteId } )
...
function publishServiceProvider.canAddCommentsToService( publishSettings )
    ...
    return PixoristAPI.testPixoristConnection( publishSettings )
...
function publishServiceProvider.addCommentToPublishedPhoto( publishSettings, remotePhotoId, commentText )
    ...
    local success = PixoristAPI.addComment( publishSettings, {
...
PixoristPublishSupport = publishServiceProvider
